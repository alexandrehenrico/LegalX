  ...
  const handleSaveItem = (data: Revenue | Expense, type: 'revenue' | 'expense') => {
    try {
      if (selectedItem) {
        ...
        if (type === 'revenue') {
          const updatedRevenue = localStorageService.updateRevenue(selectedItem.id, data as Partial<Revenue>);
          if (updatedRevenue) {
            console.log('Receita atualizada com sucesso');
          }
        } else {
          const updatedExpense = localStorageService.updateExpense(selectedItem.id, data as Partial<Expense>);
          if (updatedExpense) {
        ...
        // Criar novo item
        if (type === 'revenue') {
          const newRevenue = localStorageService.saveRevenue(data as Omit<Revenue, 'id'>);
          console.log('Nova receita criada com sucesso');
        } else {
          const newExpense = localStorageService.saveExpense(data as Omit<Expense, 'id'>);
